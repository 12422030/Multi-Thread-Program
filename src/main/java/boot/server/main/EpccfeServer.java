...
public class EpccfeServer implements Runnable {
	...
	private EpccfeServer() {
		
	}
	
	public EpccfeServer(String serverType, String oprType) {
	...
	public static void main(String[] args) {
		...
		if (args == null || args.length == 0 || StringUtils.isEmpty(args[0]) || "start".equals(args[0])) {
    		...
    		Thread syncHttpThread = new Thread(new EpccfeServer("http", "start"));
        	syncHttpThread.start();

    		Thread syncTcpThread = new Thread(new EpccfeServer("tcp", "start"));
        	syncTcpThread.start();
		} else if (args.length == 1 && "stop".equals(args[0])) {
    		Thread syncHttpThread = new Thread(new EpccfeServer("http", "stop"));
        	syncHttpThread.start();

    		Thread syncTcpThread = new Thread(new EpccfeServer("tcp", "stop"));
        	syncTcpThread.start();
		}
	}
