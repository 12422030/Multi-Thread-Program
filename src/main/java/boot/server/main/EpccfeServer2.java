...
public class EpccfeServer2 implements Runnable {
	...
	private EpccfeServer2() {
		
	}
	
	public EpccfeServer2(String serverType, String oprType) {
...
//    		Thread syncHttpThread = new Thread(new EpccfeServer2("http", "start"));
//        	syncHttpThread.start();

    		Thread syncTcpThread = new Thread(new EpccfeServer2("tcp", "start"));
        	syncTcpThread.start();
		} else if (args.length == 1 && "stop".equals(args[0])) {
    		Thread syncHttpThread = new Thread(new EpccfeServer2("http", "stop"));
        	syncHttpThread.start();

    		Thread syncTcpThread = new Thread(new EpccfeServer2("tcp", "stop"));
        	syncTcpThread.start();
		}
	}
