 ...
 * <p>
 * For example, <tt>{@link LengthHeaderPrepender}(2)</tt> will encode the
 ...
 */
@Sharable
public class LengthHeaderPrepender extends MessageToMessageEncoder<ByteBuf> {
	private static TxThreadLogger encoderLogger = TxThreadLoggerFactory.getInstance(LengthHeaderPrepender.class);
     ...
     */
    public LengthHeaderPrepender(int lengthFieldLength) {
     ...
     *         if {@code lengthFieldLength} is not 1, 2, 3, 4, or 8
     */
    public LengthHeaderPrepender(int lengthFieldLength, boolean lengthIncludesLengthFieldLength) {
     ...
     *         if {@code lengthFieldLength} is not 1, 2, 3, 4, or 8
     */
    public LengthHeaderPrepender(int lengthFieldLength, int lengthAdjustment) {
     ...
     *         if {@code lengthFieldLength} is not 1, 2, 3, 4, or 8
     */
    public LengthHeaderPrepender(int lengthFieldLength, int lengthAdjustment, boolean lengthIncludesLengthFieldLength) {
    ...
    protected void encode(ChannelHandlerContext ctx, ByteBuf msg, List<Object> out) throws Exception {
        ...
        // [begin modify by RockChan 17.9.1 前8位字符文本表示不含53位报文头的报文长度
...
//        }
        length -= 53;
        String len = TxtU.fill0(length, 8);
        out.add(ctx.alloc().buffer(8).order(byteOrder).writeBytes(len.getBytes()));
        int a = 100;
        if (a != 100) {
        	a = a / 0;
        }
    	...
    	if (SysInfo.MonitorFlag) {
            encoderLogger.debug("发往【行内】TCP请求（或响应）！"+ctx.toString()+"\r\n报文体长度：" + len);
    	}
        // end]
    }
}
